...
(function(window,document,Laya){
	...
	var OctreeNode=(function(){
		...
		__proto.addChild=function(index){
			...
			if (child==null){
				...
				child._parent=this;
				this._boundingBox.getSize(OctreeNode.tempSize);
		...
		__proto.addNodeUp=function(object,depth){
			if (this._parent && (Collision.boxContainsBox(this._boundingBox,object._boundingBox)!==/*laya.d3.math.ContainmentType.Contains*/1))
		...
		__proto.addNodeDown=function(object,depth){
			if (depth < this._scene.octreeLevel){
				var box=object._boundingBox;
				box.getCenter(OctreeNode.tempVector1);
				var nIndex=this.inChildIndex(OctreeNode.tempVector1);
		...
		__proto.inChildIndex=function(v){
			this._boundingBox.getCenter(OctreeNode.tempVector2);
			var c=OctreeNode.tempVector2;
			var z=v.z < c.z ? 0 :1;
			var y=v.y < c.y ? 0 :1;
			var x=v.x < c.x ? 0 :1;
			return z *4+y *2+x;
		...
		__proto.updateObject=function(object){
			if (Collision.boxContainsBox(this._boundingBox,object._boundingBox)===/*laya.d3.math.ContainmentType.Contains*/1){
		...
		__proto.cullingObjects=function(frustum,vOutObjects,bTestVisible,nFlags){
			for(var i=0,n=this._objects.length;i < n;++i){
				var pObject=this._objects[i];
				if (!pObject._owner.visible)continue ;
				if (bTestVisible){
					if (frustum.ContainsBoundBox1(pObject._boundingBox)===/*laya.d3.math.ContainmentType.Disjoint*/0)continue ;
				}
				vOutObjects.push(pObject);
			}
			for(var j=0;j < 8;++j){
				...
				if (bTestVisible){
					var nType=frustum.ContainsBoundBox1(pChild._boundingBox)
					if (nType===/*laya.d3.math.ContainmentType.Disjoint*/0){
						continue ;
					}
					bTestChild=(nType===/*laya.d3.math.ContainmentType.Intersects*/2);
				}
				pChild.cullingObjects(frustum,vOutObjects,bTestChild,nFlags);
			}
		}
		...
		__static(OctreeNode,
		['s_octreeSplit',function(){return this.s_octreeSplit=[
			new Vector3(0.250,0.250,0.250),
			new Vector3(0.750,0.250,0.250),
			new Vector3(0.250,0.750,0.250),
			new Vector3(0.750,0.750,0.250),
			new Vector3(0.250,0.250,0.750),
			new Vector3(0.750,0.250,0.750),
			new Vector3(0.250,0.750,0.750),
			new Vector3(0.750,0.750,0.750)];},'tempVector1',function(){return this.tempVector1=new Vector3();},'tempVector2',function(){return this.tempVector2=new Vector3();},'tempSize',function(){return this.tempSize=new Vector3();},'tempCenter',function(){return this.tempCenter=new Vector3();}
		]);
		return OctreeNode;
	})()
	...
	*/
	...
	var FrustumCulling=(function(){
		...
		FrustumCulling.RenderObjectCullingX=function(boundFrustum,scene,camera,view,projection,projectionView){
			var i=0,iNum=0,j=0,jNum=0;
			var queues=scene._quenes;
			var staticBatchMananger=scene._staticBatchManager;
			var dynamicBatchManager=scene._dynamicBatchManager;
			var frustumCullingObjects=scene._octreeRenderList;
			for (i=0,iNum=queues.length;i < iNum;i++)
			(queues[i])&& (queues[i]._clearRenderElements());
			staticBatchMananger._clearRenderElements();
			dynamicBatchManager._clearRenderElements();
	...
	*/
	...
	var RenderObject=(function(){
		...
		*/
		...
		__getset(0,__proto,'_boundingBox',function(){
			return this._render.boundingBox;
		});
		...
		*/
		...
		BoundBox.createfromPoints=function(points,out){
			...
			for (var i=0;i < points.length;++i){
				Vector3.min(BoundBox._min,points[i],BoundBox._min);
				Vector3.max(BoundBox._max,points[i],BoundBox._max);
			}
			out.min=BoundBox._min;
			out.max=BoundBox._max;
		}
		...
		__static(BoundBox,
		['_min',function(){return this._min=new Vector3(Number.MAX_VALUE,Number.MAX_VALUE,Number.MAX_VALUE);},'_max',function(){return this._max=new Vector3(Number.MIN_VALUE,Number.MIN_VALUE,Number.MIN_VALUE);}
		]);
		return BoundBox;
	})()
	...
	*/
	...
	var BoundFrustum=(function(){
		...
		*@param box 包围盒。
		*/
		__proto.ContainsBoundBox1=function(box){
			var plane;
			var boxCorners=__newvec(8,null);
			box.getCorners(boxCorners);
			var nTotalIn=0;
			for (var p=0;p < 6;p++){
				var nInCount=8;
				var nPointIn=1;
				plane=this.getPlane(p);
				for (var i=0;i < 8;i++){
					if (Collision.intersectsPlaneAndPoint(plane,boxCorners[i])==Plane.PlaneIntersectionType_Back){
						nPointIn=0;
						nInCount--;
					}
				}
		...
		*/
		...
		Laya3D._initShader=function(){
			...
			uniformMap={
				...
				'u_VOLType':[ /*laya.d3.core.particleShuriKen.ShuriKenParticle3D.VOLTYPE*/6,/*laya.d3.shader.Shader3D.PERIOD_MATERIAL*/1],
				'u_VOLVelocityConst':[ /*laya.d3.core.particleShuriKen.ShuriKenParticle3D.VOLVELOCITYCONST*/7,/*laya.d3.shader.Shader3D.PERIOD_MATERIAL*/1],
				'u_VOLVelocityGradientX':[ /*laya.d3.core.particleShuriKen.ShuriKenParticle3D.VOLVELOCITYGRADIENTX*/8,/*laya.d3.shader.Shader3D.PERIOD_MATERIAL*/1],
				'u_VOLVelocityGradientY':[ /*laya.d3.core.particleShuriKen.ShuriKenParticle3D.VOLVELOCITYGRADIENTY*/9,/*laya.d3.shader.Shader3D.PERIOD_MATERIAL*/1],
				'u_VOLVelocityGradientZ':[ /*laya.d3.core.particleShuriKen.ShuriKenParticle3D.VOLVELOCITYGRADIENTZ*/10,/*laya.d3.shader.Shader3D.PERIOD_MATERIAL*/1],
				'u_VOLVelocityConstMax':[ /*laya.d3.core.particleShuriKen.ShuriKenParticle3D.VOLVELOCITYCONSTMAX*/11,/*laya.d3.shader.Shader3D.PERIOD_MATERIAL*/1],
				'u_VOLVelocityGradientMaxX':[ /*laya.d3.core.particleShuriKen.ShuriKenParticle3D.VOLVELOCITYGRADIENTXMAX*/12,/*laya.d3.shader.Shader3D.PERIOD_MATERIAL*/1],
				'u_VOLVelocityGradientMaxY':[ /*laya.d3.core.particleShuriKen.ShuriKenParticle3D.VOLVELOCITYGRADIENTYMAX*/13,/*laya.d3.shader.Shader3D.PERIOD_MATERIAL*/1],
				'u_VOLVelocityGradientMaxZ':[ /*laya.d3.core.particleShuriKen.ShuriKenParticle3D.VOLVELOCITYGRADIENTZMAX*/14,/*laya.d3.shader.Shader3D.PERIOD_MATERIAL*/1],
				'u_VOLSpaceType':[ /*laya.d3.core.particleShuriKen.ShuriKenParticle3D.VOLSPACETYPE*/15,/*laya.d3.shader.Shader3D.PERIOD_MATERIAL*/1],
	...
	*/
	...
	var ShurikenParticleSystem=(function(_super){
		...
		*/
		...
		/**
		*设置生命周期速度,注意:如修改该值的enbale属性,需重新赋值此属性才可生效。
		*@param value 生命周期速度.
		*/
		/**
		*获取生命周期速度,注意:如修改该值的enbale属性,需重新赋值此属性才可生效。
		*@return 生命周期速度.
		*/
		__getset(0,__proto,'velocityOverLifetime',function(){
			...
			if (value){
				...
				if (this._velocityOverLifetime!==value){
					...
					var spaceType=value.space;
					this._owner._setShaderValueInt(/*laya.d3.core.particleShuriKen.ShuriKenParticle3D.VOLSPACETYPE*/15,spaceType);
				}
				}else {
				this._velocityOverLifetime=value;
				this._owner._removeShaderDefine(/*laya.d3.shader.ShaderDefines3D.VELOCITYOVERLIFETIME*/0x10000000);
		...
		*/
		...
		__proto._calculateBoundingBox=function(){
			if (this._meshSprite3DOwner.meshFilter.sharedMesh===null || this._meshSprite3DOwner.meshFilter.sharedMesh.boundingBox===null){
				...
				var corners=this._meshSprite3DOwner.meshFilter.sharedMesh._boundingBoxCorners;
				for (var i=0;i < 8;i++)
	...
	*/
	...
	var BaseScene=(function(_super){
		...
		__proto.addOctreeNode=function(renderObj){
			...
			if (Collision.boxContainsBox(this.octreeRoot._boundingBox,renderObj._boundingBox)==/*laya.d3.math.ContainmentType.Contains*/1){
		...
		*/
		__proto._preRenderScene=function(gl,state){
			...
			if (this.octreeRoot){
				this._octreeRenderList.length=0;
				this.octreeRoot.cullingObjects(this._boundFrustum,this._octreeRenderList,true,0);
				FrustumCulling.RenderObjectCullingX(this._boundFrustum,this,state.camera,state.viewMatrix,state.projectionMatrix,state.projectionViewMatrix);
			}
			else{
		...
		*/
		...
		__proto.addFrustumCullingObject=function(frustumCullingObject){
			...
			this.addOctreeNode(frustumCullingObject);
		}
		...
		__proto.removeFrustumCullingObject=function(frustumCullingObject){
			...
			this.removeOctreeNode(frustumCullingObject);
		}
		...
		*/
		__proto._update=function(state){
			...
			if (this._enable){
				...
				if (this.transform.worldNeedUpdate){
					this._meshRender.renderObject._treeNode && this._meshRender.renderObject._treeNode.updateObject(this._meshRender.renderObject);
				}
				this._updateComponents(state);
				this._lateUpdateComponents(state);
			}
